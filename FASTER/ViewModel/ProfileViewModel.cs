    ...
    internal class ProfileViewModel
    {
        ...
        private string SetCommandLine()
        {
            ...
            List<string> arguments = new List<string>
            {
                ...
                $" \"-profiles={Path.Combine(Properties.Settings.Default.serverPath, "Servers", Profile.Id)}\"",
                $" -name={Profile.Id}",
                $"{(!string.IsNullOrWhiteSpace(playerMods) || Profile.ContactDLCChecked || Profile.GMDLCChecked || Profile.PFDLCChecked || Profile.CLSADLCChecked ? $" \"-mod={(Profile.ContactDLCChecked ? "contact;" : "")}{(Profile.GMDLCChecked ? "GM;" : "")}{(Profile.PFDLCChecked ? "vn;" : "")}{(Profile.CLSADLCChecked ? "csla;" : "")}{(!string.IsNullOrWhiteSpace(playerMods) ? playerMods + ";" : "" )}\"" : "")}",
